...
try:
    ...
    st.error(
        "Could not import `data_utils` or `visualization` modules. Please ensure they are in the correct path."
    )

    # Create dummy functions to allow the app to run without crashing
    def dummy_func(*args, **kwargs):
        st.warning(f"A function was called but its module could not be loaded.")
        if "df" in kwargs:
            return kwargs["df"]
    ...
    load_predictions_file = calculate_prediction_metrics = apply_sorting_filtering = (
        calculate_image_differences
    ) = prepare_export_data = dummy_func
    display_dataset_overview = display_prediction_analysis = display_image_viewer = (
        display_image_grid
    ) = display_statistical_summary = display_model_comparison_stats = (
        display_visual_comparison
    ) = dummy_func
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["🔍 Single Run Analysis", "⚖️ Model Comparison", "🖼️ Image Gallery"]
    )
...
def render_single_run_analysis():
    ...
    with col1:
        ...
        if uploaded_file is not None:
            st.info(
                "📤 You have uploaded a file. The uploaded file will override any file selected below."
            )
        else:
            st.info(
                "📁 You can either upload a file above or select one from the list below."
            )
        ...
        if outputs_dir.exists():
            prediction_files = sorted(
                list(outputs_dir.rglob("**/predictions/submission.csv")), reverse=True
            )
            if prediction_files:
                file_options = {
                    str(f.relative_to(outputs_dir)): f for f in prediction_files
                }
    ...
    with col2:
        if predictions_file is not None:
            try:
                ...
                if df.empty:
                    st.error(
                        "The predictions file is empty. Please check your file and try again."
                    )
                    return
                ...
                if missing_columns:
                    st.error(
                        f"The predictions file is missing required columns: {', '.join(missing_columns)}. Expected columns: {', '.join(required_columns)}"
                    )
                    return

                st.success(
                    f"Successfully loaded `{getattr(predictions_file, 'name', predictions_file)}` with {len(df)} rows."
                )
                ...
                with analysis_tab3:
                    if image_dir.is_dir():
                        ...
                        st.warning(
                            "The specified image directory does not exist. Please provide a valid path."
                        )
                with analysis_tab4:
                    if image_dir.is_dir():
                        ...
                        st.warning(
                            "The specified image directory does not exist. Advanced analysis requires images."
                        )
            ...
            st.info(
                "Please upload a predictions file or select one from the outputs directory to begin analysis."
            )
...
def display_advanced_analysis(df: pd.DataFrame, image_dir: Path):
    ...
    images_per_page = st.slider(
        "Images per page", 5, 25, 10, key="advanced_images_per_page"
    )
    ...
    with col3:
        if st.button(
            "Next ➡️", key="advanced_next", disabled=current_page >= total_pages - 1
        ):
            st.session_state.advanced_page += 1
...
def render_comparison_view():
    ...
    with col1:
        ...
        uploaded_a = st.file_uploader(
            "Upload Model A Predictions", type=["csv"], key="model_a_upload"
        )
    ...
    with col2:
        ...
        uploaded_b = st.file_uploader(
            "Upload Model B Predictions", type=["csv"], key="model_b_upload"
        )
    ...
    def validate_predictions_file(file_obj):
        ...
        try:
            ...
            if df.empty:
                st.error(
                    f"The predictions file '{file_name}' is empty. Please check your file and try again."
                )
                return None
            ...
            if missing_columns:
                st.error(
                    f"The predictions file '{file_name}' is missing required columns: {', '.join(missing_columns)}. Expected columns: {', '.join(required_columns)}"
                )
                return None
    ...
    if model_a_file and model_b_file:
        ...
        if df_a is not None and df_b is not None:
            ...
            st.error(
                "Failed to load one or both prediction files. Please check the files and try again."
            )
    else:
...
def display_model_differences(df_a: pd.DataFrame, df_b: pd.DataFrame):
    ...
    with col1:
        pred_order = st.radio(
            "Order", ["Largest", "Smallest"], key="pred_order", horizontal=True
        )

    with col2:
        if st.button(
            "⬅️ Previous", key="pred_prev", disabled=st.session_state.pred_page == 0
        ):
    ...
    st.dataframe(
        sorted_pred.iloc[start_idx:end_idx][
            ["filename", "pred_a", "pred_b", "pred_diff"]
        ]
    )
    ...
    with col1:
        area_order = st.radio(
            "Order", ["Largest", "Smallest"], key="area_order", horizontal=True
        )

    with col2:
        if st.button(
            "⬅️ Previous", key="area_prev", disabled=st.session_state.area_page == 0
        ):
    ...
    st.dataframe(
        sorted_area.iloc[start_idx:end_idx][
            ["filename", "area_a", "area_b", "area_diff"]
        ]
    )
    ...
    with col1:
        conf_order = st.radio(
            "Order", ["Largest", "Smallest"], key="conf_order", horizontal=True
        )

    with col2:
        if st.button(
            "⬅️ Previous", key="conf_prev", disabled=st.session_state.conf_page == 0
        ):
    ...
    st.dataframe(
        sorted_conf.iloc[start_idx:end_idx][
            ["filename", "conf_a", "conf_b", "conf_diff"]
        ]
    )
...
def render_image_gallery():
    ...
    with col1:
        ...
        predictions_file = st.file_uploader(
            "Upload predictions CSV", type=["csv"], key="gallery_predictions"
        )
    ...
    with col2:
        try:
            ...
            filtered_df = apply_sorting_filtering(
                df, sort_by, sort_order, filter_metric
            )
            ...
            display_image_grid(
                current_page_df, str(image_dir), sort_by, images_per_page
            )
            ...
            if selected_image:
                display_image_with_predictions(
                    filtered_df, selected_image, str(image_dir)
                )
...
def render_low_confidence_analysis(df: pd.DataFrame):
    ...
    fig = px.histogram(
        df, x="avg_confidence", nbins=20, title="Low Confidence Distribution"
    )
    st.plotly_chart(fig, use_container_width=True)

    # Top issues
    st.markdown("#### Images with Lowest Confidence")
    lowest_conf = df.nsmallest(10, "avg_confidence")[
        ["filename", "avg_confidence", "prediction_count"]
    ]
