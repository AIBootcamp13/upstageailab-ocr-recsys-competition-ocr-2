...
class PreprocessingViewerPipeline:
    ...
    def process_with_intermediates(
        ...
        try:
            ...
            if config.get("enable_document_flattening", True) and isinstance(results.get("detected_corners"), np.ndarray):
                try:
                    flattened_result = self.document_flattener.flatten_document(current_image, results["detected_corners"])
                    if (
            ...
            if config.get("enable_perspective_correction", True) and isinstance(results.get("detected_corners"), np.ndarray):
                try:
                    corrected, _, _ = self.perspective_corrector.correct(current_image, results["detected_corners"])
                    current_image = corrected
                    results["perspective_corrected"] = current_image.copy()
                except Exception:
                    # Skip perspective correction if it fails
                    pass

            # Stage 5: Orientation correction
            if config.get("enable_orientation_correction", False) and isinstance(results.get("detected_corners"), np.ndarray):
                try:
                    corrected, _, _ = self.orientation_corrector.correct(current_image, results["detected_corners"])
                    current_image = corrected
                    results["orientation_corrected"] = current_image.copy()
                except Exception:
                    # Skip orientation correction if it fails
                    pass
