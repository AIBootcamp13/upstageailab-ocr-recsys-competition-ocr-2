...
def main():
    ...
    ap.add_argument('--desired_short', type=int, default=640, help='Base desired short-side after resize for median bucket')
    ap.add_argument('--tiny_boost', type=float, default=1.3, help='Multiplier for smallest bucket target size')
    ap.add_argument('--max_upscale', type=float, default=3.0, help='Cap on (target_short / original_short)')
    ap.add_argument('--round_to', type=int, default=32, help='Round target sizes to multiple of this')
    args = ap.parse_args()
    ...
    shorts = np.array([
        ...
        if (
            'short' in v
            and isinstance(v['short'], (int, float))
            and v['short'] > 0
        )
    ], dtype=np.float32)
    if shorts.size == 0:
    ...
    for edge in edges:
        ...
        if len(buckets) == 0:
            ...
            target_short = max(target_short, args.desired_short * 0.1)  # Prevent negative or too small values
